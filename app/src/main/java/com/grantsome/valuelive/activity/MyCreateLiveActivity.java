...
public class MyCreateLiveActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_my_create_live);
    ...
    private void init() {
        ...
        if(mConversationIdList.size() == 0){
            mConversationIdList = HttpUtils.findCreateConversationIdListById(mObjectId);
        }
        Log.d("MyJoinLiveActivity", "init: Id.size() == " + mConversationIdList.size());
        if(mConversationNameList.size() == 0){
            mConversationNameList = HttpUtils.findCreateConversationNameListById(mObjectId);
        }
